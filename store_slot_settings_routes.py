...
def register_store_slot_settings_routes(app):
    ...
    def store_slot_settings(store_id):
        ...
        # Google設定を取得
        cur.execute(_sql(conn, 'SELECT review_url, slot_spin_count, show_slot_review_button FROM "T_店舗_Google設定" WHERE store_id = %s'), (store_id,))
        ...
        show_slot_review_button = google_row[2] if google_row and len(google_row) > 2 and google_row[2] is not None else 1
        ...
        if request.method == 'POST':
            ...
            show_slot_review_button = 1 if request.form.get("show_slot_review_button") == "on" else 0
            ...
            if cur.fetchone():
                cur.execute(_sql(conn, '''
                    ...
                    SET review_url = %s, slot_spin_count = %s, show_slot_review_button = %s, updated_at = CURRENT_TIMESTAMP
                    WHERE store_id = %s
                '''), (google_url, slot_spin_count, show_slot_review_button, store_id))
            else:
                cur.execute(_sql(conn, '''
                    INSERT INTO "T_店舗_Google設定" (store_id, review_url, slot_spin_count, show_slot_review_button)
                    VALUES (%s, %s, %s, %s)
                '''), (store_id, google_url, slot_spin_count, show_slot_review_button))
