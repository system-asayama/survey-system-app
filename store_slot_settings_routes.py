...
def register_store_slot_settings_routes(app):
    ...
    def store_print_prizes(store_id):
        """景品一覧印刷ページ"""
        try:
            print(f"[DEBUG] store_print_prizes: store_id={store_id}")
            tenant_id = session.get('tenant_id')
            conn = get_db_connection()
            cur = conn.cursor()
            
            # 店舗情報を取得
            cur.execute(_sql(conn, 'SELECT id, name, slug FROM "T_店舗" WHERE id = %s'), (store_id,))
            store_row = cur.fetchone()
            print(f"[DEBUG] store_print_prizes: store_row={store_row}")
            ...
            cur.execute(_sql(conn, 'SELECT prizes_json FROM "T_店舗_景品設定" WHERE store_id = %s'), (store_id,))
            prizes_row = cur.fetchone()
            print(f"[DEBUG] store_print_prizes: prizes_row={prizes_row}")
            
            if prizes_row and prizes_row[0]:
                try:
                    prizes = json.loads(prizes_row[0])
                    print(f"[DEBUG] store_print_prizes: prizes loaded from DB")
                except Exception as e:
                    print(f"[ERROR] store_print_prizes: JSON parse error: {e}")
                    prizes = []
            else:
                prizes = [
                    {"min_score": 500, "rank": "🎁 特賞", "name": "特別景品"},
                ...
                print(f"[DEBUG] store_print_prizes: using default prizes")
            
            conn.close()
            print(f"[DEBUG] store_print_prizes: rendering template")
            
            return render_template('print_prizes.html', store=store, prizes=prizes)
        except Exception as e:
            print(f"[ERROR] store_print_prizes: {e}")
            import traceback
            traceback.print_exc()
