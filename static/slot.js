...
// 音声をWAVファイルとしてダウンロード
async function downloadResultSound(totalScore) {
  ...
  if (totalScore >= 300) {
    ...
    notes.forEach((freq, i) => {
      ...
      oscillator.type = 'triangle';
      oscillator.frequency.setValueAtTime(freq, i * 0.15);
      
      gainNode.gain.setValueAtTime(0.3, i * 0.15);
      gainNode.gain.exponentialRampToValueAtTime(0.01, i * 0.15 + 0.3);
      
      oscillator.start(i * 0.15);
      oscillator.stop(i * 0.15 + 0.3);
    });
  } else if (totalScore >= 150) {
    // 明るい音を生成
    const oscillator = offlineContext.createOscillator();
    ...
    oscillator.frequency.exponentialRampToValueAtTime(880, 0.3);
  ...
  a.href = url;
  a.download = `slot_result_${totalScore}points.wav`;
  a.click();
...
async function play(){
  ...
  downloadBtn.textContent = '🔊 音をダウンロード';
  downloadBtn.className = 'sub';
  downloadBtn.style.marginLeft = '10px';
  downloadBtn.onclick = () => downloadResultSound(total);
  
  const statusEl = $('#status');
  statusEl.textContent = `合計: ${total}`;
